...
const StockTicker = ({ 
  ...
  const generateChartPath = (data) => {
    ...
    const startY = points[0].y; // Y-coordinate of the first data point
  ...
  return (
    <div className={`${bgColorClass} rounded-lg p-6 w-full text-white`}>
      ...
      <div className="mb-4">
        <h2 className="text-3xl font-bold mb-1">{isIndex ? ticker : ticker}</h2>
        {!isIndex && <p className="text-sm opacity-90 truncate">{companyName}</p>}
      </div>
      ...
      {chartData.length > 0 && (
        <div className="mt-4">
          <svg viewBox={isIndex ? "0 0 1000 180" : "0 0 500 180"} className="w-full h-32">
...
const App = () => {
  ...
  const generateNoChangeChartData = (basePrice) => {
    ...
    for (let i = 0; i < points; i++) {
      // Create a sine wave pattern that returns to the starting price
